...
const TechMapSelectModal = props => {
    ...
    useEffect(()=>{
        if(visible) {
            ...
            apiGetConfigByName('techOperationsSmall')()
                .then((response) => {
                    let result = response.data;
                    setSecondTableConfig(result);
                });

            setErrorSelect({...errorSelect, style: {...errorSelect.style, display: 'none'} });
        ...
        if(techMapSelected){
            if(techMapSelected.isGroup){
                ...
                setErrorSelect({...errorSelect,
                    text: 'Вы не можете добавить группу. Выберете технологическую карту.',
    ...
    return (
        <Modal
            ...
            onCancel={onCancel}
            footer={[
                <span key="error" style={errorSelect.style}>{errorSelect.text}</span>,
        ...
        >
            <div style={{height: '400px'}}>
                <AdvancedTable
                    ...
                    onRowClick={({rowData}) => {
                        ...
                        apiGetFlatDataByConfigName('techOperationsSmall')({data: {techMapId: rowData.id}})
                            .then(response => {
            ...
            <div style={{height: '200px', marginTop: '24px'}}>
                <AdvancedTable
                    ...
                    type={'localSide'}
                    requestLoadConfig={apiGetConfigByName('techOperationsSmall')}
                />
            </div>
        </Modal>
    );
