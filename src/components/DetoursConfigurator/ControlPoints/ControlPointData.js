        ...
        async function fetchData() {
            try {
                ...
                if (params.id === 'new') {
                    ...
                    const cpResp = await apiGetFlatDataByConfigName('controlPoints')({data: {id: params.id}});
                    setInitFormObject(cpResp.data[0]);

                    const eResp = await apiGetFlatDataByConfigName('controlPointsEquipments')({data: {controlPointsId: params.id}});
                    setEquipments(eResp.data);

                    const tmResp = await apiGetFlatDataByConfigName('controlPointsTechMaps')({data: {controlPointsId: params.id}});
